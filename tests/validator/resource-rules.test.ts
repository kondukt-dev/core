...
import {
  RESOURCE_RULES,
  runResourceRules,
} from "../../src/validator/rules/resource-rules.js";
...
describe("resource rules", () => {
  ...
  it("resource-has-uri emits error when uri is empty", () => {
    ...
    expect(issues.some((i) => i.rule === "resource-has-uri" && i.severity === "error")).toBe(
      true,
    );
  });
  ...
  it("resource-has-name emits warning when name is empty", () => {
    ...
    expect(
      issues.some((i) => i.rule === "resource-has-name" && i.severity === "warning"),
    ).toBe(true);
  });
  ...
  it("resource-has-mime emits info when mimeType is missing for non-text uri", () => {
    ...
    expect(issues.some((i) => i.rule === "resource-has-mime" && i.severity === "info")).toBe(
      true,
    );
  });
