import { AGENT_TARGET_LABELS } from "./types.js";
import type { AgentTarget, ProjectAnalysis } from "./types.js";

export function renderAgentDocs(analysis: ProjectAnalysis, target: AgentTarget = "claude"): string {
  const sections: string[] = [];
  sections.push(renderTitle(analysis));
  sections.push(renderAgentNote(target));
...
/** @deprecated use renderAgentDocs */
export function renderClaudeMd(analysis: ProjectAnalysis): string {
  return renderAgentDocs(analysis, "claude");
}

function renderAgentNote(target: AgentTarget): string {
  return `> Context file for **${AGENT_TARGET_LABELS[target]}** — auto-generated by [Kondukt](https://kondukt.dev).`;
}
